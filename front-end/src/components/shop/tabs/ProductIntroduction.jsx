...
const ProductIntroduction = React.memo(() => {
  const introductionTexts = useMemo(() => [
  ...
  return (
    <div className="text-right leading-7 text-gray-700">
      {introductionTexts.map((text, index) => (
        <p key={index} className="mb-3 sm:mb-4 text-sm sm:text-base">
          {text}
        </p>
      ))}
    </div>
...
ProductIntroduction.displayName = 'ProductIntroduction';
